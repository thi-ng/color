...
#+BEGIN_SRC clojure :noweb-ref impl :noweb yes
  (defrecord RGBA [^double r ^double g ^double b ^double a]
    ...
    streams/IBuffer
    (get-buffer
        [_] (#?(:clj double-array :cljs ta/float32) [r g b a]))
    (into-buffer
        [_ dest stride idx]
      (aset ^doubles dest idx r)
      (aset ^doubles dest (+ idx 1) g)
      (aset ^doubles dest (+ idx 2) b)
      (aset ^doubles dest (+ idx 3) a)
      (+ idx stride)))
#+END_SRC
    ...
    (mix
      ...
      (let [r (bit-and (bit-shift-right col 16) 0xff)
        ...
        (Int24.
         (bit-or
          (bit-or
           (bit-shift-left (int (mm/mix r (+ (* (.-r dest) 0xff) 0.5) t)) 16)
           (bit-shift-left (int (mm/mix g (+ (* (.-g dest) 0xff) 0.5) t)) 8))
          (int (mm/mix b (+ (* (.-b dest) 0xff) 0.5) t))))))
    streams/IBuffer
    (get-buffer
        [_] (streams/get-buffer (as-rgba _)))
    (into-buffer
        [_ dest stride idx] (streams/into-buffer (as-rgba _) dest stride idx)))
         ...
         (bit-or
          (bit-or
           (bit-or
            ...
            (bit-shift-left (int (mm/mix g (+ (* (.-g dest) 0xff) 0.5) t)) 8))
           (int (mm/mix b (+ (* (.-b dest) 0xff) 0.5) t)))
          (bit-shift-left (int (mm/mix a (+ (* (.-a dest) 0xff) 0.5) t)) 24)))))
    streams/IBuffer
    (get-buffer
        [_] (streams/get-buffer (as-rgba _)))
    (into-buffer
...
#+BEGIN_SRC clojure :noweb-ref impl :noweb yes
  (defrecord HSVA [^double h ^double s ^double v ^double a]
    ...
    streams/IBuffer
    (get-buffer
        [_] (#?(:clj double-array :cljs ta/float32) [h s v a]))
    (into-buffer
        [_ dest stride idx]
      (aset ^doubles dest idx h)
      (aset ^doubles dest (+ idx 1) s)
      (aset ^doubles dest (+ idx 2) v)
      (aset ^doubles dest (+ idx 3) a)
      (+ idx stride)))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref impl :noweb yes
  (defrecord HSLA [^double h ^double s ^double l ^double a]
    ...
    streams/IBuffer
    (get-buffer
        [_] (#?(:clj double-array :cljs ta/float32) [h s l a]))
    (into-buffer
        [_ dest stride idx]
      (aset ^doubles dest idx h)
      (aset ^doubles dest (+ idx 1) s)
      (aset ^doubles dest (+ idx 2) l)
      (aset ^doubles dest (+ idx 3) a)
      (+ idx stride)))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref impl :noweb yes
  (defrecord CMYKA [^double c ^double m ^double y ^double k ^double a]
    ...
    streams/IBuffer
    (get-buffer
        [_] (#?(:clj double-array :cljs ta/float32) [c m y k a]))
    (into-buffer
        [_ dest stride idx]
      (aset ^doubles dest idx c)
      (aset ^doubles dest (+ idx 1) m)
      (aset ^doubles dest (+ idx 2) y)
      (aset ^doubles dest (+ idx 3) k)
      (aset ^doubles dest (+ idx 4) a)
      (+ idx stride)))
#+END_SRC
  ...
  (defn proxied-color-type
    ...
    (fn ctor [col]
      (reify
        ...
        m/IInvert
        (invert
            [_] (-> col (to-rgba) (m/invert) (ctor (from-rgba))))
        m/IInterpolate
        (mix
            [_ dest] (m/mix _ dest 0.5))
        (mix
        ...
        streams/IBuffer
        (get-buffer
            [_] (-> col to-rgba streams/get-buffer))
        (into-buffer
            [_ dest stride idx] (-> col to-rgba (streams/into-buffer dest stride idx))))))
#+END_SRC
