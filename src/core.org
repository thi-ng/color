...
#+BEGIN_SRC emacs-lisp :var val=""
  (replace-regexp-in-string "{{val}}" val
  ...
  (deftype RGBA [^double r ^double g ^double b ^double a]
    ...
    (as-int24
      ...
      (int24
       (bit-or
        (bit-or
         (-> r (* 0xff) int (bit-shift-left 16))
         (-> g (* 0xff) int (bit-shift-left 8)))
        (-> b (* 0xff) int))))
    (as-int32
      [_]
      (int32
    ...
    (blend
      ...
      (let [r (bit-and (bit-shift-right col 16) 0xff)
        ...
        (Int24.
         (bit-or
          (bit-or
           (bit-shift-left (int (mm/mix r (* (red dest) 0xff) t)) 16)
           (bit-shift-left (int (mm/mix g (* (green dest) 0xff) t)) 8))
          (int (mm/mix b (* (blue dest) 0xff) t)))))))
         ...
         (bit-or
          (bit-or
           (bit-or
            ...
            (bit-shift-left (int (mm/mix g (* (green dest) 0xff) t)) 8))
           (int (mm/mix b (* (blue dest) 0xff) t)))
          (bit-shift-left (int (mm/mix a (* (alpha dest) 0xff) t)) 24))))))
#+END_SRC
      ...
      (let [h2 (hue dest)
        ...
        (HSVA.
         (if (> hd 0.5)
           ...
           (mm/mix h h2 t))
         (mm/mix s (saturation dest) t)
         (mm/mix v (brightness dest) t)
         (mm/mix a (alpha dest) t)))))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref impl :noweb yes
  (deftype CSS [^String col]
    ...
    (blend
      [_ dest t] (as-css (blend (as-rgba _) dest t))))
#+END_SRC
