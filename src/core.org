...
#+BEGIN_SRC clojure :noweb-ref impl
  (deftype Int24 [^int col]
    ...
    (as-rgba
      [_]
      (RGBA.
       (* INV8BIT (bit-and (bit-shift-right col 16) 255))
       (* INV8BIT (bit-and (bit-shift-right col 8) 255))
       (* INV8BIT (bit-and col 255))
       1.0))
    IHSVConvert
    (as-hsva
      [_] (as-hsva (as-rgba _)))
    ...
    ICSSConvert
    (as-css
      [_] (f/format hex6 (bit-and col 0xffffff)))
    IIntConvert
    (as-int24 [_] _)
    (as-int32
    ...
    IColorComponents
    (red
      [_] (* INV8BIT (bit-and (bit-shift-right col 16) 255)))
    (green
      [_] (* INV8BIT (bit-and (bit-shift-right col 8) 255)))
    (blue
      [_] (* INV8BIT (bit-and col 255)))
    (alpha [_] 1)
    (hue
      [_] (hue (as-hsva _)))
    (saturation
  ...
  (deftype Int32 [^int col]
    ...
    (as-rgba
      ...
      (RGBA.
       ...
       (* INV8BIT (bit-and (bit-shift-right col 8) 255))
       (* INV8BIT (bit-and col 255))
       (* INV8BIT (bit-and (unsigned-bit-shift-right col 24) 255))))
    IHSVConvert
    (as-hsva
      [_] (as-hsva (as-rgba _)))
    IHSLConvert
    ...
    (blue
      [_] (* INV8BIT (bit-and col 255)))
    (alpha
      [_] (* INV8BIT (bit-and (unsigned-bit-shift-right col 24) 255)))
    (hue
      [_] (hue (as-hsva _)))
    (saturation
      [_] (saturation (as-rgba _)))
    ...
    (adjust-alpha
      [_ offset]
      (let [a (* INV8BIT (bit-and (unsigned-bit-shift-right col 24) 255))
            a (int (* 255 (m/clamp (+ a offset) 0.0 1.0)))]
...
#+BEGIN_SRC clojure :noweb-ref impl
  (deftype CMYKA [^double c ^double m ^double y ^double k ^double a]
    ...
    IColorOps
    (rotate-hue
      [_ theta] (-> _ (as-hsva) (rotate-hue theta) (as-cmyk)))
    (adjust-saturation
      [_ offset] (-> _ (as-hsva) (adjust-saturation offset) (as-cmyk)))
    (adjust-brightness
      [_ offset] (-> _ (as-hsva) (adjust-brightness offset) (as-cmyk)))
    (adjust-alpha
  ...
  (defn proxied-color-type
    ...
    (fn ctor [col]
      (reify
        ...
        IColorOps
        (rotate-hue
          [_ theta] (-> col (to-rgba) (rotate-hue theta) (ctor (from-rgba))))
        (adjust-saturation
          [_ offset] (-> col (to-rgba) (adjust-saturation offset) (ctor (from-rgba))))
        (adjust-brightness
          [_ offset] (-> col (to-rgba) (adjust-brightness offset) (ctor (from-rgba))))
        (invert
...
#+BEGIN_SRC clojure :noweb-ref ctors
  ...
  (defn int32
    ([col] (Int32. col))
    ([col alpha]
     (let [a (int (* 255 (m/clamp alpha 0.0 1.0)))]
       (Int32. (bit-or (bit-and col 0xffffff) (bit-shift-left a 24))))))
...
#+BEGIN_SRC clojure :noweb-ref conv-helpers
  ...
  (defn- hex->rgba
    ...
    (let [hex (if (= \# (first hex)) (subs hex 1) hex)
      ...
      (if (== 3 len)
        (let [[r g b] hex]
          (as-rgba (int24 (f/parse-int (str r r g g b b) 16 0))))
        (if (< len 7)
          (as-rgba (int24 (f/parse-int hex 16 0)))
          (as-rgba (int32 (f/parse-int hex 16 0)))))))
...
#+BEGIN_SRC clojure :noweb-ref const
  ...
  (def ns-prefix "#thi.ng.color.core.")
#+END_SRC
